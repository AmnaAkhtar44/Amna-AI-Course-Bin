...
df.plot.scatter(x='Radio', y='Newspaper', title='Scatter Plot of Radio and Newspaper percentages');
plt.show()
...
print(" df['Scores'] :     " , df['Newspaper'])
print("  df['Hours']   :    ", df['Radio']   )

y = df['Newspaper'].values.reshape(-1, 1)
X = df['Radio'].values.reshape(-1, 1)

print("y :  " , y)
print("X :   " , X)

print(df['Radio'].values) # [2.5 5.1 3.2 8.5 3.5 1.5 9.2 ... ]
print(df['Radio'].values.shape) # (25,)
...
Newspaper = calc(regressor.coef_, regressor.intercept_, 9.5)
print(Newspaper) # [[94.80663482]]
Newspaper = regressor.predict([[9.5]])
print(Newspaper) # 94.80663482
