...
print(ids)
print(price)
print(long) # long means longitude north-south position on the earth.
print(lat) # lat means latitude east-west position on th earth.
...
pricePie = (price/np.pi) +1 # pie is used to convert degree in radian and +1 is used to get results max of 1 not 0 at any case
...
# Splicing array # array[row_start:row_end, col_start:col_end]
...
D2LongLatSlice2=  D2LongLat[:1, 4:15:4] #:1 → still first row only.

#4:15:4 → slice columns from index 4 to 14, taking every 4th element (step = 4).
print("Zameen.com Long Plus Lat - 2 dimentional arrary - Splicing array - D2LongLat[:1, 4:15:4] " , D2LongLatSlice2)



# Indexing array # get rows,columns by their indexing value means 0 is first row and 1 is second column
D2LongLatSliceItemOnly=  D2LongLatSlice[0,1] # 0 is x index and 1 is y index
...
for elem in np.nditer(D2LongLat): #Simple iteration over all elements
	#	Iterate and track positions in array in gives values irrespective of their indexes

    print(elem)

#EDIT: If you need indexes (as a tuple for 2D table), then:
for index, elem in np.ndenumerate(D2LongLat):
    np.ndenumerate()  # Iterate and track positions in array and also give index numbers
    print(index, elem)
...
D2LongLat1TO298 = np.reshape(D2LongLat, (1, 298)) # means 2*149 is 2 rows and each row has 149 elements and in next 2 rows ans 298 elements total
